...
// SVG social media icons - render as proper vector icons, not emoji
...
function getSocialSvg(platform: string): React.ReactNode {
  ...
  if (SOCIAL_SVGS[key]) return SOCIAL_SVGS[key];
  // Partial match
  for (const [k, v] of Object.entries(SOCIAL_SVGS)) {
...
function SocialIcon({ social }: { social: Record<string, unknown> }) {
  const platform = str(social.platform) || str(social.icon) || str(social.label);
